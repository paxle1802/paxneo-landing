...
export async function buildPdf({ kind, order }) {
  ...
  if (kind === 'RECEIPT') {
    ...
    T('Please pay by bank transfer (details in your enrollment email).', L, y - 20, 11, F, ink);
    T('Access is delivered by email once payment is confirmed.', L, y - 36, 11, F, muted);
  }
...
export function bankHtml(lang) {
  ...
  return `<div style="border:1px solid #e3e3ee;border-radius:10px;padding:14px 16px;margin:16px 0;font-size:14px">
    ...
    <div style="margin-bottom:10px">
      ...
      Bank: VietinBank &nbsp;|&nbsp; ${vi ? 'Số TK' : 'Account'}: 60657888 &nbsp;|&nbsp; ${vi ? 'Chủ TK' : 'Holder'}: Le Tat Thanh
    </div>
    <div>
      <div style="color:#7c5cff;font-weight:bold;font-size:12px">${vi ? 'KHÁCH QUỐC TẾ (USD)' : 'INTERNATIONAL (USD)'}</div>
      ${vi ? 'Chủ TK' : 'Holder'}: Tat Thanh Le &nbsp;|&nbsp; ${vi ? 'Số TK' : 'Account'}: 30000001070030<br>
      ACH: 028000024 &nbsp;|&nbsp; Wire: 021000021 &nbsp;|&nbsp; SWIFT/BIC: CHASUS33<br>
      JP Morgan Chase Bank, N.A. - 383 Madison Avenue, New York, NY 10179
    </div>
  </div>`;
}
