...
export const BANK_DETAILS = {
  vn: { bank: 'Standard Chartered', account: '66-4546248-01', holder: 'Le Tat Thanh' },
  intl: {
...
export const CUSTOMER_CC = ['paxle86@gmail.com'];
// Contact emails printed on all PDF documents (invoice, receipt, agreement).
export const DOC_EMAILS = ['support@paxneo.net', 'paxle86@gmail.com'];
...
export async function sendEmail(payload) {
  if (useSmtp()) {
    try {
      // Plain-text alternative improves deliverability (multipart/alternative).
      const text = payload.text || (payload.html ? payload.html.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : undefined);
      const info = await transport().sendMail({
        from: MAIL_FROM, to: payload.to, cc: payload.cc, replyTo: payload.reply_to,
        subject: payload.subject, html: payload.html, text,
