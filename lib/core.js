...
export async function buildAgreementPdf({ student, courseName, fee, duration = TRAINING_DURATION, date }) {
  ...
  para('By signing below, both parties acknowledge and agree to the terms of this Agreement. The training fee stated above corresponds to the amount paid by the Student for the service described.', L, y, 10, F, muted);

  // Signatures — sign in the space above each line.
  const sy = 150, c1 = L, c2 = 320, cw = 190;
  line(sy, c1, c1 + cw); line(sy, c2, c2 + cw);
  T('Provider signature', c1, sy - 14, 9, F, muted);
  T(PROVIDER.legalName, c1, sy - 30, 11, B, ink);
  T('Date: ' + date, c1, sy - 45, 9, F, muted);
  T('Student signature / proof of agreement', c2, sy - 14, 9, F, muted);
  T(student.name, c2, sy - 30, 11, B, ink);
  T('Date: ______________', c2, sy - 45, 9, F, muted);
